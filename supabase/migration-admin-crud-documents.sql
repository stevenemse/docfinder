...
--    (titre caviardé, région, ville, zone, statut). Valeurs par défaut :
--    inchangé. Le statut est validé contre l'enum doc_status.
...
--    Chaque action est journalisée dans audit_logs (acteur horodaté).
-- ==============================================================================
...
CREATE OR REPLACE FUNCTION public.admin_delete_document(p_kind TEXT, p_doc_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object('masked_url', v_masked_url, 'vault_path', v_vault_path)
  );
